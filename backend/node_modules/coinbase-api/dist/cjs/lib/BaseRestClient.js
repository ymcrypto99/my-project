...
const jwtNode_js_1 = require("./jwtNode.js");
...
class BaseRestClient {
    ...
    constructor(restClientOptions = {}, networkOptions = {}) {
        ...
        const VERSION = '0.1.0';
        const USER_AGENT = `${requestUtils_js_1.APIIDPrefix}/${VERSION}`;
        ...
        this.baseUrl = (0, requestUtils_js_1.getRestBaseUrl)(restClientOptions, this.getClientType());
        ...
        this.getClientType() !== requestUtils_js_1.REST_CLIENT_TYPE_ENUM.commerce &&
    ...
    generateNewOrderId() {
        return requestUtils_js_1.APIIDPrefix + (0, nanoid_1.nanoid)(14);
    }
    /**
    ...
    validateOrderId(params, orderIdProperty) {
        ...
        if (!requestParams[orderIdProperty].startsWith(requestUtils_js_1.APIIDPrefix)) {
            (0, requestUtils_js_1.logInvalidOrderId)(orderIdProperty, requestUtils_js_1.APIIDPrefix, params);
            const previousValue = requestParams[orderIdProperty];
            const newValue = requestUtils_js_1.APIIDPrefix + requestParams[orderIdProperty];
            console.warn(`WARNING: "${orderIdProperty}" was automatically prefixed. Changed from "${previousValue}" to "${newValue}". To avoid this, apply the prefix before submitting an order or use the client.generateNewOrderId() utility method.`);
     ...
     */
    async signRequest(data, url, endpoint, method, signMethod) {
        ...
        if (signMethod === 'coinbase') {
            ...
            const signRequestParams = method === 'GET' || method === 'DELETE'
                ? (0, requestUtils_js_1.serializeParams)(data?.query || data, strictParamValidation, encodeQueryStringValues, '?', explodeArrayParameters)
                : requestBodyString;
            // https://docs.cdp.coinbase.com/product-apis/docs/welcome
            switch (clientType) {
                case requestUtils_js_1.REST_CLIENT_TYPE_ENUM.advancedTrade:
                case requestUtils_js_1.REST_CLIENT_TYPE_ENUM.coinbaseApp: {
                    ...
                    const sign = (0, jwtNode_js_1.signJWT)({
                        url,
                        method,
                        algorithm: 'ES256',
                ...
                case requestUtils_js_1.REST_CLIENT_TYPE_ENUM.exchange: {
                    ...
                    const signInput = timestampInSeconds + method + endpoint + signRequestParams;
                ...
                case requestUtils_js_1.REST_CLIENT_TYPE_ENUM.international: {
                    const timestampInSeconds = timestampInMs / 1000; // decimals are OK
                    if (!apiSecret) {
                        throw new Error(`No API secret provided, cannot sign request.`);
                    }
                    ...
                    const signInput = timestampInSeconds + method + endpoint + requestBodyString;
                    const sign = await (0, webCryptoAPI_js_1.signMessage)(signInput, apiSecret, 'base64', 'SHA-256', 'base64:web');
                    const headers = {
                        'CB-ACCESS-KEY': apiKey,
                        'CB-ACCESS-SIGN': sign,
                ...
                case requestUtils_js_1.REST_CLIENT_TYPE_ENUM.prime: {
                ...
                case requestUtils_js_1.REST_CLIENT_TYPE_ENUM.commerce: {
                ...
                default: {
                    console.error(new Date(), (0, misc_util_js_1.neverGuard)(clientType, `Unhandled sign client type : "${clientType}"`));
                    throw new Error(`Unhandled request sign for client : "${clientType}"`);
                }
            }
        }
    ...
    async prepareSignParams(method, url, endpoint, signMethod, params, isPublicApi) {
        if (isPublicApi) {
            return {
    ...
    async buildRequest(method, endpoint, url, params, isPublicApi) {
        ...
        const signResult = await this.prepareSignParams(method, url, endpoint, 'coinbase', params, isPublicApi);
