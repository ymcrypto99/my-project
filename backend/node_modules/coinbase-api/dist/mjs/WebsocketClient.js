...
import { isCBAdvancedTradeErrorEvent, isCBAdvancedTradeWSEvent, isCBExchangeWSEvent, isCBExchangeWSRequestOperation, isCBINTXWSRequestOperation, isCBPrimeWSRequestOperation, } from './lib/websocket/typeGuards.js';
import { getCBExchangeWSSign, getCBInternationalWSSign, getCBPrimeWSSign, getMergedCBExchangeWSRequestOperations, WS_KEY_MAP, WS_URL_MAP, } from './lib/websocket/websocket-util.js';
...
export class WebsocketClient extends BaseWebsocketClient {
    ...
    async getWsOperationEventsForTopics(topicRequests, wsKey, operation) {
        ...
        switch (wsKey) {
            ...
            case WS_KEY_MAP.exchangeMarketData:
            case WS_KEY_MAP.exchangeDirectMarketData: {
                if (!operationEvents.every((evt) => isCBExchangeWSRequestOperation(evt, wsKey))) {
                ...
                const mergedOperationEvents = getMergedCBExchangeWSRequestOperations(operationEvents);
                ...
                if (!maxTopicsPerEvent ||
                    ...
                    const { sign, timestampInSeconds } = await getCBExchangeWSSign(apiSecret);
            ...
            case WS_KEY_MAP.internationalMarketData: {
                if (!operationEvents.every((evt) => isCBINTXWSRequestOperation(evt, wsKey))) {
                    // Don't expect this to ever happen, but just to please typescript...
                    throw new Error(`Unexpected request schema for exchange WS request builder`);
                }
                // We're over the max topics per request limit. Break into batches.
                ...
                for (const operationEvent of operationEvents) {
                    ...
                    const { sign, timestampInSeconds } = await getCBInternationalWSSign(apiKey, apiSecret, apiPassphrase);
                    const wsRequestEventWithSign = {
                        ...operationEvent,
            ...
            case WS_KEY_MAP.primeMarketData: {
                if (!operationEvents.every((evt) => isCBPrimeWSRequestOperation(evt, wsKey))) {
                    // Don't expect this to ever happen, but just to please typescript...
                    throw new Error(`Unexpected request schema for exchange WS request builder`);
                }
                if (!apiKey || !apiSecret || !apiPassphrase) {
